...
var use_gltf_model := true
var gltf_arms_model: Node3D = null
var arms_skeleton: Skeleton3D = null
...
func _load_gltf_arms():
        var model_path = "res://assets/art_pack2/Characters/Base Characters/Godot/Superhero_Male.gltf"
        
        if not ResourceLoader.exists(model_path):
                push_warning("[FirstPersonArms] glTF model not found, using procedural arms")
                use_gltf_model = false
                _create_realistic_arms()
                return
        ...
        if err != OK:
                push_warning("[FirstPersonArms] Failed to load glTF: " + str(err))
                use_gltf_model = false
                _create_realistic_arms()
                return
        
        gltf_arms_model = gltf_doc.generate_scene(gltf_state)
        if not gltf_arms_model:
                push_warning("[FirstPersonArms] Failed to generate scene from glTF")
                use_gltf_model = false
                _create_realistic_arms()
                return
        
        right_arm = Node3D.new()
        right_arm.name = "RightArmContainer"
        add_child(right_arm)
        
        left_arm = Node3D.new()
        left_arm.name = "LeftArmContainer"
        add_child(left_arm)
        
        right_arm.add_child(gltf_arms_model)
        gltf_arms_model.position = Vector3(-0.28, -1.15, -0.15)
        gltf_arms_model.scale = Vector3(0.8, 0.8, 0.8)
        gltf_arms_model.rotation_degrees = Vector3(0, 180, 0)
        
        _find_skeleton(gltf_arms_model)
        _hide_non_arm_parts()
        _setup_arm_materials()
        _setup_item_holder_from_bone()
        
        print("[FirstPersonArms] glTF arms loaded successfully")
...
func _hide_non_arm_parts():
        if not gltf_arms_model:
                return
        
        var parts_to_hide := ["Head", "Chest", "Leg", "Foot", "Pelvis", "Spine", "neck"]
        
        for mesh in _find_all_mesh_instances(gltf_arms_model):
                var mesh_name = mesh.name.to_lower()
                var should_hide = false
                for part in parts_to_hide:
                        if part.to_lower() in mesh_name:
                                should_hide = true
                                break
                if should_hide:
                        mesh.visible = false
...
func _setup_item_holder_from_bone():
        if arms_skeleton:
                var hand_bone_idx = arms_skeleton.find_bone("hand_r")
                if hand_bone_idx >= 0:
                        var bone_attach = BoneAttachment3D.new()
                        bone_attach.name = "HandAttachment"
                        bone_attach.bone_name = "hand_r"
                        arms_skeleton.add_child(bone_attach)
                        
                        item_holder = Node3D.new()
                        item_holder.name = "ItemHolder"
                        item_holder.position = Vector3(0.02, 0.05, 0.02)
                        bone_attach.add_child(item_holder)
                        return
