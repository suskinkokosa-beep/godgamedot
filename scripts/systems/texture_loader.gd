...
func load_texture(texture_id: String, texture_type: String = "nature") -> Texture2D:
        ...
        if not tex_dict.has(texture_id):
                ...
                return _create_fallback_texture(texture_id)
...
func _create_fallback_texture(texture_id: String) -> Texture2D:
        var fallback_key = "fallback_" + texture_id
        if texture_cache.has(fallback_key):
                return texture_cache[fallback_key]
        
        var img = Image.create(64, 64, false, Image.FORMAT_RGBA8)
        var color = _get_fallback_color(texture_id)
        img.fill(color)
        
        var texture = ImageTexture.create_from_image(img)
        texture_cache[fallback_key] = texture
        return texture

func _get_fallback_color(texture_id: String) -> Color:
        if "bark" in texture_id or "wood" in texture_id:
                return Color(0.4, 0.25, 0.15)
        elif "leaves" in texture_id or "grass" in texture_id:
                return Color(0.2, 0.5, 0.2)
        elif "rock" in texture_id or "stone" in texture_id:
                return Color(0.5, 0.5, 0.5)
        elif "metal" in texture_id:
                return Color(0.6, 0.6, 0.65)
        elif "flower" in texture_id:
                return Color(0.8, 0.3, 0.4)
        elif "sand" in texture_id or "desert" in texture_id:
...
func load_texture_set(texture_id: String, texture_type: String = "nature") -> Dictionary:
        ...
        if not tex_dict.has(texture_id):
                return {"base": _create_fallback_texture(texture_id)}
        ...
        if result.is_empty():
                result["base"] = _create_fallback_texture(texture_id)
