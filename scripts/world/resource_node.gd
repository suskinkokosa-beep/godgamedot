...
func gather(gatherer) -> Dictionary:
        ...
        if inv and is_instance_valid(inv) and inv.has_method("add_item"):
                var added = inv.add_item(resource_type, int(amount), data.get("weight", 1.0))
                if added:
                        _show_notification(gatherer, "gathered", int(amount))
        
        if gatherer and is_instance_valid(gatherer):
                var prog = get_node_or_null("/root/PlayerProgression")
                if prog and is_instance_valid(prog):
                        var pid = 1
                        if "net_id" in gatherer:
                                pid = gatherer.net_id
                        if prog.has_method("add_skill_xp"):
                                prog.add_skill_xp(pid, data.get("xp_skill", "gathering"), data.get("xp_amount", 1.0) * amount)
                        if prog.has_method("add_xp"):
                                prog.add_xp(pid, data.get("xp_amount", 1.0) * 0.5 * amount)
        
        _play_gather_sound()
        _spawn_gather_effect()
        _animate_hit()
        
        emit_signal("gathered", resource_type, amount, gatherer)
        
        if current_amount <= 0:
                call_deferred("_on_depleted")
        
        return {"success": true, "type": resource_type, "amount": int(amount), "remaining": current_amount}

func _play_gather_sound():
        ...
        match resource_type:
                "wood", "log", "stick":
                        if audio.has_method("play_hit_sound"):
                                audio.play_hit_sound("wood")
                "stone", "iron_ore", "copper_ore", "silver_ore", "gold_ore", "titanium_ore":
                        if audio.has_method("play_hit_sound"):
                                audio.play_hit_sound("stone")
                _:
                        if audio.has_method("play_item_pickup"):
                                audio.play_item_pickup()

func _spawn_gather_effect():
        var vfx = get_node_or_null("/root/VFXManager")
        if vfx and is_instance_valid(vfx) and vfx.has_method("spawn_item_pickup_effect"):
